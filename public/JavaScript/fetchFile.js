...
// To Auto Send fetch request for each filePath in recentFilePaths array
window.addEventListener("load", function () {
   Promise.all(recentFilePaths.map((filePath) => {
      return fetch('/server/file/read', {
            method: 'POST',
...
// 6. 📄 READFILE On Click On File Manager 📄
document.addEventListener("click", function (event) {
...
// 🗿 Ace editor element
function createAceElement(parent, fileExt) {
   const editor = document.createElement('div');
   editor.style.width = '100%';
   editor.style.height = '100%';
   parent.appendChild(editor);
   ...
   switch (fileExt) {
      ...
      case 'css':
         ...
         break;
      case 'java':
         aceInstance.session.setMode('ace/mode/java');
         break;
      default:
         aceInstance.session.setMode('ace/mode/text');
         break;
   }
   return aceInstance;
}
...
// Add format code button
   ...
   formatBtn.classList.add('format-btn');
   formatBtn.addEventListener('click', () => {
      const session = aceInstance.getSession();
      const code = session.getValue();
      const formattedCode = window.beautify(code, { indent_size: 2 });
      session.setValue(formattedCode);
   });
   const fileMenu = document.getElementById('FileMenu');   
   fileMenu.appendChild(formatBtn);
...
function createTextFile(data, filePath) {
  ...
  // Add Ace editor element
  const aceEditorElement = createAceElement(fileContent, fileExt);
  if (data) {
    aceEditorElement.setValue(data);
    aceEditorElement.focus();
  }
  ...
  saveBtn.addEventListener('click', () => {
    const textdata = aceEditorElement.getValue();
    if (typeof textdata !== 'string') {
      throw new Error('Expected textdata to be a string but it is not.');
    }
    fetch('/save-file', {
      method: 'POST',
      body: JSON.stringify({
